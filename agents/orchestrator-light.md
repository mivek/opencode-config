...
- Authoring the fix yourself when the user or a reviewer flags an issue. Relay the **finding** to `@planner` and let it design the fix; don't hand over a settled solution (e.g. "refactor X to take Y") that leaves the planner no judgment. Deciding the approach is `@planner`'s job — you route the finding, the planner revises, `@momus` re-checks.
